...
rw_work_done_df$SiteCode = sapply(rw_work_done_df$SiteCode, park_to_peit_code) #This renames them all to the Peit style Names using ROSETTA
...
reduced_survey_df <- survey_df[survey_df$SiteCode %in% unique(rw_work_done_df$SiteCode), ] #reducing the survey DF to sites were we have work done
...
top_sum_survey_df <- top_survey_sites %>% 
  ...
  filter(SppCode == "BOAT" | SppCode == "ANIS" | SppCode == "NOTO") %>%
  summarize(Count = sum(Count))
...
ggplot() +
  ...
  facet_wrap(~ SiteCode) +
  labs(x = "Date", y = "Count", title = "Top Sites, Invertebrate Count vs Time") +
  scale_x_date(date_labels = "%Y-%m-%d")
...
ggplot() +
  ...
  geom_vline(data = all_amphib_rw_work_done_df, aes(xintercept = Date_End, color = Generic_Description), linewidth=.75) + 
  scale_color_manual(values = brewer.pal(n=10, name= "Spectral")) + 
  #geom_vline(aes(xintercept = Date_Start, color = Work_Done), linetype = "solid", linewidth=.75) + 
  facet_wrap(~ SiteCode) +
